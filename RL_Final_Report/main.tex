...
\begin{itemize}
		\item \textbf{Pricing:} For each pending request, the price to offer the customer
		\item \textbf{Repositioning:} For each vehicle, the desired (x, y) location to move towards
		\item \textbf{Dispatching:} For each request, a one-hot encoded vector indicating which vehicle to dispatch (or none)
\end{itemize}
\paragraph{Transition Model} The environment's transition dynamics were modeled to reflect realistic interactions:
\begin{itemize}
...
physical deployment. We propose next steps to improve performance in the final section.
...
\begin{itemize}
		\item \textbf{Observation Space Improvements:} There are a handful of redundant, shared, or noisy variables
				in the observation space that could be removed or refined to improve learning efficiency. For example,
				active rides duplicate information from pending requests. In addition, the (x, y) coordinates could be
				replaced with learned embeddings or graph-based representations to better capture spatial relationships.
		\item \textbf{Neural Network Improvements:} There are many areas of exploration in the neural network
				architecture. These include improved hierarchical modeling of each sub-agent, improved priors to
				encourage desired behavior, and learned embeddings of categorical variables. The advantages to a
				hierarchical agent approach include (1) individul sub-agent pre-training (2) reduced observation space
				per sub-agent, discarding irrelevant information, and (3)compatibility and enrichment with heuristic agents.

				There are many parallel decisions being taken by the agent at each time step that could be better
				modeled in a sequential manner, or at least whose outputs' score are conditional on the other output. For
				example, the score awarded to the reposition decision of vehicle B is conditional on the reposition
				decision of vehicle A. A reasonable prior on the desired distribution of repositioned vehicles could be a
				uniform distribution across the graph, scaled by the (learned) "importance" of each node. Instead of
				calculating repositioning actions sequentially, the repositioning sub-agent could be encouraged to
				reposition vehicles uniformly over the graph. An entropic regularization term (amongst others) could
				achieve this sub-goal.

				Finaly, categorical variables (e.g., weather state) could be represented as learned embeddings instead of
				one-hot encodings, allowing the model to learn relationships between categories.
		\item \textbf{Reward Improvements:} The reward structure, conditioned on the buckets outlined above, could
				benefiting from rebalancing. For example, the reward structure for dispatching actions is strictly
				negative, ostensibly encouraing the agent to avoid dispatching entirely.
		\item \textbf{Action Space Improvements:} From observing the reward logs, and the Neural Network outputs,
				it's become apparent that the dispatching action space is complicated. Mapping requests to vehicles
				directly creates a combinatorial explosion in the action space, making learning difficult (and is another
				case of a poor system encouraging sequential decision-making). An alternative approach could be to have
				the agent output a score for each vehicle-request pair, and then use a matching algorithm (e.g.,
				Hungarian algorithm) to assign vehicles to requests based on these scores.
\end{itemize}
